...
export function Card({ className = '', children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={`rounded-2xl border border-blue-100 bg-white text-slate-800 shadow-sm ${className}`}
      {...props}
    >
      {children}
    </div>
...
export function CardFooter({ className = '', children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={`flex items-center p-5 pt-0 border-t border-blue-50/50 mt-4 ${className}`} {...props}>
      {children}
    </div>
  );
}
